...
import { share, tap, delay, concatMap } from 'rxjs/operators';
...
describe('tests the onChange$ ', () => {
  ...
  test.only('inner obvservable can be interupted with takeUntil when field blurs', done => {
    ...
    const fakeOnBlur = of(1).pipe(concatMap(x => of(x).pipe(delay(100, scheduler))));
  ...
  /* Doesn't need a virtualTimeScheduler just needs to make sure that the catchError clause
    is run when an error is thrown, time is already abstracted out of the problem via the nature 
    of streams so virtualTimeScheduler would just complicate the test wihout making it stronger
  */
  test(
  ...
  /* Didn't bother using virtualScheduler here because time isn't a relavent factor
     just need to test that a validation function can return a promise and pass throug
     the observable stream, or emit an error( done in a seperate test)
  */
  test(
